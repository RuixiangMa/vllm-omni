...
Usage:
    python openai_asr_client.py [audio_path] [--url URL]

Examples:
    python openai_asr_client.py                                    # Use default audio
    python openai_asr_client.py /path/to/audio.wav                 # Specify audio file
    python openai_asr_client.py /path/to/audio.wav --url http://localhost:8000  # Custom URL
"""
...
def test_transcription(audio_path: str, base_url: str = "http://localhost:8000"):
    ...
    prompt_text = f"This is a {duration:.2f} seconds audio, please transcribe it with these keys: " + ", ".join(
        show_keys
    )
                ...
                if line:
                    ...
                    if decoded_line.startswith("data: "):
                        ...
                        try:
                            ...
                            # Ignore malformed JSON chunks in the streaming response
...
def main():
    parser = argparse.ArgumentParser(description="Test VibeVoice vLLM API with streaming output")
    parser.add_argument(
        "audio_path", nargs="?", default=None, help="Path to audio file (wav, mp3, flac, etc.) or video file"
    )
    parser.add_argument(
        "--url", default="http://localhost:8004", help="vLLM server base URL (default: http://localhost:8004)"
    )
