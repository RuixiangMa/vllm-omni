...
def get_cache_backend(cache_backend: str | None, cache_config: Any) -> CacheBackend | None:
    ...
    - faster_cache: Uses FasterCacheBackend with enable()/refresh() interface

    Args:
        cache_backend: Cache backend name ("cache_dit", "tea_cache", "faster_cache", or None).
        cache_config: Cache configuration (dict or DiffusionCacheConfig instance).

    Returns:
        Cache backend instance (CacheDiTBackend, TeaCacheBackend, or FasterCacheBackend) if cache_backend is set,
        None otherwise.
    ...
    elif cache_backend == "faster_cache":
        return FasterCacheBackend(cache_config)
    else:
        raise ValueError(f"Unsupported cache backend: {cache_backend}. Supported: 'cache_dit', 'tea_cache', 'faster_cache'")