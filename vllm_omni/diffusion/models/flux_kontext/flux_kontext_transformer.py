...
# Original implementation: https://github.com/huggingface/diffusers/blob/main/src/diffusers/models/transformers/transformer_flux.py
...
class FluxKontextAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        """
        RoPE is applied uniformly to all queries and keys.
        """
        qkv_output, _ = self.to_qkv(hidden_states)
        q_size = self.to_qkv.num_heads * self.head_dim
        kv_size = self.to_qkv.num_kv_heads * self.head_dim
        query, key, value = qkv_output.split([q_size, kv_size, kv_size], dim=-1)
...
class FluxKontextTransformerBlock(nn.Module):
    def __init__(
        ...
        self.attn = FluxKontextAttention(
            query_dim=dim,
